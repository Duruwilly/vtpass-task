...
const Home = () => {
 const { servicesData, identifier, isLoading, handleDataClick, getServicesData } = useGlobalContext()
  ...
  return (
    <section className="flex justify-center items-center">
      <div className="w-full max-w-screen-md h-screen px-4 mt-7">
        {identifier === 'electricity-bill' ? ( <h2>Select Your Disco</h2> ) : identifier === 'education' ? (<h2>Select Examination Type</h2>) : identifier === 'funds' ? (<h2>Select Bank</h2>) : (<h2>Select Mobile Operator</h2>)}
              ...
              <div className="relative" key={index}>
                <label className="relative container">
                  <input
                    ...
                    onClick={() => {
                      handleImageClick(services.serviceID, services.image, services.name);
